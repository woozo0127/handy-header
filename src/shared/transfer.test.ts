...
describe('parseProfileFile', () => {
  ...
  it('빈 이름은 Imported profile로 대체한다', () => {
    const data = JSON.parse(serializeProfile(sampleProfile()));
    data.profile.name = '  ';
    expect(parseProfileFile(JSON.stringify(data)).name).toBe(
      'Imported profile',
    );
  });
  ...
  it('type이 다르면 Not a HandyHeader profile file 에러를 던진다', () => {
    expect(() =>
      parseProfileFile(JSON.stringify({ type: 'other', version: 1 })),
    ).toThrow('Not a HandyHeader profile file');
    expect(() => parseProfileFile(JSON.stringify(['array']))).toThrow(
  ...
  it('version이 다르면 Unsupported file version 에러를 던진다', () => {
    const data = JSON.parse(serializeProfile(sampleProfile()));
    data.version = 2;
    expect(() => parseProfileFile(JSON.stringify(data))).toThrow(
  ...
  it('규칙 필드 형식이 잘못되면 Malformed profile data 에러를 던진다', () => {
    ...
    expect(() => parseProfileFile(JSON.stringify(noProfile))).toThrow(
      'Malformed profile data',
    );

    const badDirection = base();
    badDirection.profile.headerRules[0].direction = 'both';
    expect(() => parseProfileFile(JSON.stringify(badDirection))).toThrow(
      'Malformed profile data',
    );

    const missingValue = base();
    delete missingValue.profile.headerRules[0].value;
    expect(() => parseProfileFile(JSON.stringify(missingValue))).toThrow(
      'Malformed profile data',
    );

    const notArray = base();
    notArray.profile.redirectRules = 'nope';
    expect(() => parseProfileFile(JSON.stringify(notArray))).toThrow(
      'Malformed profile data',
    );
  });
});
...
describe('uniqueProfileName', () => {
  ...
  it('접미사 붙은 이름도 이미 있으면 숫자를 증가시킨다', () => {
    expect(
      uniqueProfileName('Staging', ['Staging', 'Staging (2)', 'Staging (3)']),
    ).toBe('Staging (4)');
  });
});
