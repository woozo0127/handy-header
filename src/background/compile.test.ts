...
function header(over: Partial<HeaderRule> = {}): HeaderRule {
  return { id: 'h1', enabled: true, direction: 'request', name: 'X-Test', value: '1', ...over }
}
...
function stateWith(rules: { headerRules?: HeaderRule[]; redirectRules?: RedirectRule[] }): AppState {
...
describe('compile', () => {
  ...
  it('request 헤더 룰을 modifyHeaders/requestHeaders set으로 매핑한다', () => {
    const rules = compile(stateWith({ headerRules: [header({ name: 'X-Debug-Mode', value: 'true' })] }))
    expect(rules).toEqual([
      {
        ...
        condition: { resourceTypes: expect.arrayContaining(['main_frame', 'xmlhttprequest']) },
      },
    ])
  })
  ...
  it('response 헤더 룰은 responseHeaders로 매핑한다', () => {
    const rules = compile(
      stateWith({ headerRules: [header({ direction: 'response', name: 'Access-Control-Allow-Origin', value: '*' })] }),
    )
  ...
  it('리다이렉트 룰을 regexFilter/regexSubstitution으로 변환한다', () => {
    ...
    expect(rules[0].condition.regexFilter).toBe('^https://api\\.example\\.com/(.*)$')
  })

  it('match의 regex 특수문자를 이스케이프한다', () => {
    const rules = compile(
      stateWith({ redirectRules: [redirect({ match: 'https://a.com/x?y=1*', target: 'https://b.com/*' })] }),
    )
    expect(rules[0].condition.regexFilter).toBe('^https://a\\.com/x\\?y=1(.*)$')
  })

  it('다중 와일드카드는 순서대로 캡처·치환한다', () => {
    const rules = compile(
      stateWith({
        redirectRules: [redirect({ match: 'https://*.example.com/api/*', target: 'http://localhost/*/v2/*' })],
      }),
    )
    expect(rules[0].condition.regexFilter).toBe('^https://(.*)\\.example\\.com/api/(.*)$')
    expect(rules[0].action).toEqual({
      type: 'redirect',
  ...
  it('target의 *가 match보다 많으면 제외한다', () => {
    const rules = compile(
      stateWith({ redirectRules: [redirect({ match: 'https://a.com/*', target: 'https://b.com/*/*' })] }),
    )
    expect(rules).toEqual([])
  })
  ...
  it('match 또는 target이 비어 있으면 제외한다', () => {
    const rules = compile(
      stateWith({
        redirectRules: [redirect({ id: 'a', match: '' }), redirect({ id: 'b', target: '' })],
      }),
    )
    expect(rules).toEqual([])
  })
