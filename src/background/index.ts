...
import { STATE_KEY, loadState, saveCompileError } from '../shared/storage'
...
async function applyBadge(text: string): Promise<void> {
  ...
  if (text) {
    await chrome.action.setBadgeBackgroundColor({ color: text === 'ERR' ? '#D93025' : '#26A756' })
  }
}
...
async function sync(): Promise<void> {
  ...
  try {
    ...
    await applyBadge(badgeText({ enabled: state.globalEnabled, ruleCount: rules.length, error: false }))
  } catch (err) {
