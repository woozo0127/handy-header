...
export type AppActions = {
  ...
  updateRedirectRule: (
    id: string,
    patch: Partial<Omit<RedirectRule, 'id'>>,
  ) => void;
    ...
    const mutateActive = (fn: (p: Profile) => Profile) => {
      update((prev) => ({
        ...
        profiles: prev.profiles.map((p) =>
          p.id === prev.activeProfileId ? fn(p) : p,
        ),
      }));
    };
    ...
    return {
      ...
      addProfile: () =>
        update((prev) => {
          const profile = createDefaultProfile(
            `Profile ${prev.profiles.length + 1}`,
          );
          return {
            ...prev,
        ...
        update((prev) => {
          const named = {
            ...
            name: uniqueProfileName(
              profile.name,
              prev.profiles.map((p) => p.name),
            ),
          };
          return {
            ...prev,
            profiles: [...prev.profiles, named],
            activeProfileId: named.id,
          };
        }),
      ...
      addHeaderRule: (direction) => {
        ...
        mutateActive((p) => ({
          ...
          headerRules: [
            ...p.headerRules,
            { id, enabled: true, direction, name: '', value: '' },
          ],
        }));
        return id;
      },
      ...
      updateHeaderRule: (id, patch) =>
        mutateActive((p) => ({
          ...p,
          headerRules: p.headerRules.map((r) =>
            r.id === id ? { ...r, ...patch } : r,
          ),
        })),
      ...
      removeHeaderRule: (id) =>
        mutateActive((p) => ({
          ...p,
          headerRules: p.headerRules.filter((r) => r.id !== id),
        })),
      ...
      updateRedirectRule: (id, patch) =>
        mutateActive((p) => ({
          ...p,
          redirectRules: p.redirectRules.map((r) =>
            r.id === id ? { ...r, ...patch } : r,
          ),
        })),
      ...
      removeRedirectRule: (id) =>
        mutateActive((p) => ({
          ...p,
          redirectRules: p.redirectRules.filter((r) => r.id !== id),
        })),
    };
  }, []);
